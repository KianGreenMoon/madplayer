# Madplayer — the design

> **This is the client's own design doc.** It lived in madshare's repo as
> `docs/ui/madplayer.md` until 2026-08-17, when most of it moved here (owner's
> call): the design of one client is that client's business. What stayed behind
> is the basic shape plus the parts madshare itself is answerable for — the
> rules the server computes that no client may re-derive
> (§"What the server already computes" there) and the access-model reasoning its
> own code cites. The **behavioural contracts both UIs follow stay in madshare**
> — `docs/ui/library-page.md`, `docs/ui/player-and-queue.md`,
> `docs/ui/artists-and-performers.md`, `docs/ui/madnetwork-page.md`, and the
> architecture docs — and changing one of those is still a madshare commit.
> Changing THIS doc is a madplayer commit.
>
> **Reference convention:** paths of the form `docs/ui/…`, `docs/architecture/…`
> and `docs/plans/…` name madshare's docs. A madshare checkout sits beside this
> repository (`../madshare` — the same layout the go.mod `replace` requires), so
> they resolve from here as `../madshare/docs/…`.
...
>
> **madplayer lives in its own repository** (split out at madshare v0.8.6),
> beside madshare's. It requires madshare as an ordinary Go module pinned to a
...
The full argument **stays in madshare's `docs/ui/madplayer.md`** under this
same title, because madshare's own code cites it (`app/network.go`). The short
form: a listener node never exchanges keys, so no community forms around it —
`serveAudience` finds no friend row, no member, no placeable token issuer, and
the guest fallback is off by default. The whole publishing model is **inert
here**, not disabled; the one switch that would expose the library is
...
**This section stays in madshare's `docs/ui/madplayer.md`**, in full, under this
same title — it is the cross-client contract half of the old doc, cited by
madshare's own facade (`app/library.go`) and by `docs/architecture/embedding.md`.
Read it there before writing any list code: every rule in its table (which names
are artists, which file plays, ladder order, disc grouping, sort order,
availability, the default version pick, playlist re-pointing, cover variants) is
decided server-side, and a client that re-derives one quietly disagrees with the
web UI about what the library contains.
...
## Level 2b, concretely (designed 2026-08-09; built by 2026-08-15)
