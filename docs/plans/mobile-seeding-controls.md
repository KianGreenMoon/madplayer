...
Status: **draft, 2026-08-22.** Lives in madplayer because the work is almost
entirely client-side (madshare only verifies a round-trip, M5). A separate
task from **madshare's** `docs/plans/full-node-mode.md` on purpose (owner —
that plan changes madshare's facade contract, so it stays over there): that
plan is about desktop
