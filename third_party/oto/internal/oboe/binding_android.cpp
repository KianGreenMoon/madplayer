...
const char *Stream::Play(int sample_rate, int channel_num,
  ...
  if (!stream_) {
    ...
    // LOCAL PATCH (madplayer): PerformanceMode::None, not LowLatency.
    //
    // Upstream asks for the low-latency path, which Android's OpenSL layer
    // turns into AUDIO_OUTPUT_FLAG_FAST|AUDIO_OUTPUT_FLAG_RAW: a FAST mixer
    // track on an output stream whose HAL buffer is 128 frames — 2.7 ms.
    // Measured on a Pixel 7 Pro, 2026-08-18, one 60 s window per row, same
    // track, same speaker:
    //
    //   FAST|RAW  (this app, upstream oto)  HAL buffer 128   40 underruns/min
    //   DEEP_BUFFER (the phone's browser)   HAL buffer 960    0 underruns/min
    //
    // Each underrun was ~72 frames of silence, so ~40 clicks a minute: the
    // "crackle" the owner had been reporting for days. It is NOT this
    // process being late — over the same window AudioFlinger's fast mixer
    // found our track full on every single cycle (Empty and Partial did not
    // move), and the stream underran 15 times a minute even while the app
    // was PAUSED and feeding silence. The 2.7 ms buffer is simply below what
    // this device's HAL sustains.
    //
    // A music player has no use for the low-latency path and every use for
    // the deep one, which is where PerformanceMode::None lands
    // (SL_ANDROID_PERFORMANCE_NONE -> AUDIO_OUTPUT_FLAG_DEEP_BUFFER). The
    // price is output latency, which matters to a synthesiser and not to a
    // player whose controls all go through a mixer it clears anyway.
